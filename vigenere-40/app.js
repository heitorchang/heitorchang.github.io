...
function ptoc(p, k) {
  return itoc((ctoi(p) + ctoi(k)) % 40);
}

// Ciphertext (c) to Plaintext (p) with Key (k)
function ctop(c, k) {
  return itoc((ctoi(c) - ctoi(k)) % 40);
}
...
function encrypt(plaintext, key) {
  errors = "";
  const encrypted = "# " + groupBy(processToStr(plaintext, key, 1), 4) + " #";
  if (errors) {
