...
def clear_console():
  ...
  term = $("body").terminal(interpreter, {
    greetings: 'Python 3.11.3 (Pyodide Core) is ready.',
    prompt: ps1,
...
function sendToInterpreter(py, switch_focus) {
  ...
  logical_blocks.forEach((block) => {
    // if `block` is a single line, remove the trailing newline
    if (block.substring(0, block.length - 1).indexOf('\n') === -1) {
      block = block.replace(/\n$/, "");
    }
    term.exec(block);
  });
