...
let inputHistoryFromStorage = window.localStorage.getItem("pyReplHistory") || "[]"
...
var input = CodeMirror.fromTextArea(replInput, {
  ...
  indentUnit: 4,
  extraKeys: {
    // "Shift-Enter": () => { pyEval(input) },
    "Ctrl-Enter": () => { pyEval(input) },
    "Alt-Enter": () => { pyEval(input, true) },
...
// heitor edit custom keys
CodeMirror.keyMap.default["Shift-Tab"] = "indentLess";

input.setSize("100%", "calc(100% - 3.5rem)")

const pyErrorMsg = document.getElementById("pyError")

// PyScheme functions
function setErrorCursor() {
  const newValue = input.getValue().trim();
  input.setValue(newValue);
  input.setCursor(newValue.length);
}
...
function pyEval(input, clearInput = false) {
  const inputValue = input.getValue();
  inputHistory.unshift(inputValue)
  inputHistoryIndex = -1
  window.localStorage.setItem("pyReplHistory", JSON.stringify(inputHistory.slice(0, 50)))
  // eval
  sendToInterpreter(input.getValue(), false);
...
// Book content keypress listener
