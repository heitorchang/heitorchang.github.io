...
function ptoc(p, k) {
  return itoc((ctoi(p) + ctoi(k)) % charlist.length);
}

// Ciphertext (c) to Plaintext (p) with Key (k)
function ctop(c, k) {
  return itoc((ctoi(c) - ctoi(k)) % charlist.length);
}
...
function handleDecrypt() {
  ...
  var input = document.getElementById("ciphertext").value.trim().replace(/ /g, "").replace(/\n/g, "");
  var key = normalizeKey(document.getElementById("key").value);
  document.getElementById("key").value = key;
